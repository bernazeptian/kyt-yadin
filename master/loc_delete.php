...
$id = (int)($_GET['id'] ?? 0);
if (!$id) { header('Location: index.php?tab=locations'); exit; }

$check = $pdo->prepare("SELECT id FROM hiyari_reports WHERE location_id = :id LIMIT 1");
$check->execute([':id' => $id]);
if ($check->fetch()) { header('Location: index.php?tab=locations&error=in_use'); exit; }

try {
    $pdo->prepare("DELETE FROM locations WHERE id = :id")->execute([':id' => $id]);
    header('Location: index.php?tab=locations&success=loc_deleted');
