...
try {
        ...
        foreach ($_FILES['photos']['tmp_name'] as $key => $tmp) {
            ...
            if (move_uploaded_file($tmp, $dest)) {
                $photoStmt = $pdo->prepare("
                    INSERT INTO report_photos (report_id, file_path, created_at)
                    VALUES (:report_id, :file_path, NOW())
                ");
                $photoStmt->execute([
                    ':report_id'  => $report_id,
    ...
    error_log('Hiyari store error: ' . $e->getMessage());
    header('Location: create.php?error=db');
    exit;
}
?>
