...
$year  = date('Y');
$month = date('m');
$count = $pdo->query("SELECT COUNT(*) FROM hiyari_reports WHERE report_type = 'kiken' AND YEAR(created_at) = $year AND MONTH(created_at) = $month")->fetchColumn();
$report_number = 'KY-' . $year . $month . '-' . str_pad($count + 1, 3, '0', STR_PAD_LEFT);
...
<head>
  ...
  <title>New Kiken Yochi Report</title>
  ...
  <style>
    .ky-badge { display:inline-block;background:#f39c12;color:#fff;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700;margin-left:10px;vertical-align:middle; }
  </style>
</head>
<body>
  <aside class="sidebar">
    <div class="sidebar__logo"><img src="../assets/logo.png" alt="Yanmar" class="sidebar__logo-img" /></div>
    <nav class="sidebar__nav">
      <a href="../index" class="nav-item" data-tooltip="Dashboard">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg>
      </a>
      <a href="index" class="nav-item nav-item--active" data-tooltip="Reports">
  ...
  <main class="main">
    <header class="topbar">
      <div class="topbar__left">
        ...
        <div>
          <h1 class="topbar__title">New Kiken Yochi Report <span class="ky-badge">危険予知</span></h1>
          <p class="topbar__sub">Unsafe act & unsafe condition reporting</p>
        </div>
      </div>
      <div class="topbar__right">
        <div class="topbar__date"><?php echo date('l, d F Y'); ?></div>
        <div class="topbar__avatar" id="avatarBtn"><?php echo strtoupper(substr($_SESSION['name'] ?? 'U', 0, 1)); ?></div>
        <div class="avatar-dropdown" id="avatarDropdown">
          <div class="avatar-dropdown__info">
            <div class="avatar-dropdown__avatar"><?php echo strtoupper(substr($_SESSION['name'] ?? 'U', 0, 1)); ?></div>
            <div>
    ...
    <div class="content">
      ...
      <?php if ($error): ?>
        <div class="alert alert--error">
          ...
          <?php $msgs=['1'=>'Something went wrong.','db'=>'Database error.']; echo $msgs[$error]??'An error occurred.'; ?>
        </div>
      <?php endif; ?>

      <form method="POST" action="store_kiken" enctype="multipart/form-data" id="reportForm">
        <?php echo csrf_field(); ?>
        <input type="hidden" name="report_type" value="kiken" />
          ...
          <div class="form-col">
            ...
            <div class="form-section">
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <input type="text" class="form-input form-input--readonly" name="report_number" value="<?php echo htmlspecialchars($report_number); ?>" readonly />
                ...
                <div class="form-group">
                  ...
                  <input type="date" class="form-input" id="report_date" name="report_date" value="<?php echo date('Y-m-d'); ?>" required />
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <select class="form-select" id="department_id" name="department_id" required onchange="filterLocations(this.value)">
                ...
                <div class="form-group">
                  ...
                  <select class="form-select" id="location_id" name="location_id" required>
                    ...
                    <?php foreach ($locations as $loc): ?>
                      <option value="<?php echo $loc['id']; ?>" data-dept="<?php echo $loc['department_id']; ?>" style="display:none">
              ...
              <!-- Category - Unsafe Act or Unsafe Condition only -->
              <div class="form-group">
                <label class="form-label">Category <span class="required">*</span></label>
                <div class="category-group">
                  <label class="category-option">
                    <input type="radio" name="category" value="unsafe_action" required />
              ...
              <div class="form-group">
                ...
                <textarea class="form-textarea" id="description" name="description" rows="5" placeholder="Describe what was observed, where, and potential hazard..." required></textarea>
              </div>

              <div class="form-group">
                <label class="form-label" for="safe_action">Safe Action (Tindakan Karantina) <span class="required">*</span></label>
                <textarea class="form-textarea" id="safe_action" name="safe_action" rows="4" placeholder="Describe the immediate containment action taken..." required></textarea>
              </div>
            ...
            <div class="form-section">
              <div class="form-section__header">
                ...
                <h2 class="form-section__title">Photos <span class="form-section__optional">(optional)</span></h2>
              </div>
              <div class="upload-area" id="uploadArea">
                <input type="file" id="photos" name="photos[]" multiple accept="image/*" class="upload-input" />
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="36" height="36" class="upload-icon"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
            ...
            <div class="form-section form-section--sticky">
              ...
              <div class="form-group">
                ...
                <div class="risk-options">
                  <?php foreach (['A'=>'Almost Certain','B'=>'Likely','C'=>'Moderate','D'=>'Unlikely','E'=>'Rare'] as $val=>$label): ?>
                    <label class="risk-option">
                      <input type="radio" name="likelihood" value="<?php echo $val; ?>" <?php echo $val==='A'?'required':''; ?> />
                      <span class="risk-btn"><span class="risk-code"><?php echo $val; ?></span><span class="risk-label"><?php echo $label; ?></span></span>
                    </label>
                  <?php endforeach; ?>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="risk-options">
                  <?php foreach ([1=>'Insignificant',2=>'Minor',3=>'Moderate',4=>'Major',5=>'Catastrophic'] as $val=>$label): ?>
                    <label class="risk-option">
                      <input type="radio" name="severity" value="<?php echo $val; ?>" />
                      <span class="risk-btn"><span class="risk-code"><?php echo $val; ?></span><span class="risk-label"><?php echo $label; ?></span></span>
                    </label>
                  <?php endforeach; ?>
                </div>
              ...
              <div class="risk-result" id="riskResult">
                <div class="risk-result__placeholder" id="riskPlaceholder">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="28" height="28"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              ...
              <div class="matrix-wrap">
                ...
                <div class="matrix-scroll">
                  <table class="matrix-table">
                    ...
                    <tbody>
                      <?php
                      $matrix=['A'=>['E','E','E','E','E'],'B'=>['M','H','H','E','E'],'C'=>['L','M','H','E','E'],'D'=>['L','L','M','H','E'],'E'=>['L','L','M','H','H']];
                      ...
                      foreach ($matrix as $row=>$cols): ?>
                        <tr>
                          ...
                          <?php foreach ($cols as $s=>$val): ?>
                            <td class="matrix-cell <?php echo $classMap[$val]; ?>" data-l="<?php echo $row; ?>" data-s="<?php echo $s+1; ?>"><?php echo $val; ?></td>
                          <?php endforeach; ?>
                        </tr>
                      <?php endforeach; ?>
                    </tbody>
        ...
        <div class="form-actions">
          ...
          <button type="submit" class="btn btn--submit" id="submitBtn" style="background:#f39c12;">
  ...
  function filterLocations(deptId) {
    ...
    options.forEach(opt => {
      if (opt.dataset.dept === deptId) { opt.style.display = ''; hasOptions = true; }
      else { opt.style.display = 'none'; }
    });
    locSelect.value = '';
    locSelect.querySelector('option:first-child').textContent = hasOptions ? '-- Select Location --' : '-- No locations for this department --';
  }
  </script>
